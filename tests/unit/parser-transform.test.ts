...
import { discoverTurns, inferRole, parseTurn } from '../../packages/parser-dom/src/index.js';
...
describe('parser helpers', () => {
  ...
  it('parses structured blocks', () => {
    const dom = new JSDOM(
      '<article data-role="assistant"><h2>Heading</h2><p>Body</p><ul><li>One</li></ul></article>'
    );
  ...
  it('prefers strict turn selectors and avoids container over-capture', () => {
    const dom = new JSDOM(`
      <main>
        <article id="container">
          <article data-message-author-role="user"><div data-message-content><p>User text</p></div></article>
          <article data-message-author-role="assistant"><div data-message-content><p>Assistant text</p></div></article>
        </article>
      </main>
    `);

    const turns = discoverTurns(dom.window.document);
    expect(turns).toHaveLength(2);
    expect(turns.every((node) => node.hasAttribute('data-message-author-role'))).toBe(true);
  });

  it('selects markdown/body wrapper before block extraction', () => {
    const dom = new JSDOM(`
    ...
    const turnEl = dom.window.document.querySelector('article');
    const turn = parseTurn(turnEl!);
    expect(turn.blocks.map((block) => block.kind)).toEqual(['heading', 'paragraph']);
  });
});
