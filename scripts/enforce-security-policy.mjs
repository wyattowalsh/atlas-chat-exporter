...
const ADAPTER_ROOTS = ['apps/extension', 'apps/userscript', 'apps/bookmarklets', 'apps/snippets', 'apps/cli'];
const MANIFEST_NAMES = new Set(['package.json', 'pnpm-lock.yaml', 'package-lock.json', 'yarn.lock']);
...
async function run() {
  ...
  const forbiddenEndpointRegexes = networkPolicy.forbiddenEndpointPatterns.map((pattern) => new RegExp(pattern, 'i'));
  const allowedHosts = new Set(networkPolicy.defaultAdapterPolicy.allowedHosts ?? []);
  ...
  for (const adapterRoot of ADAPTER_ROOTS) {
    ...
    await walk(adapterRoot, async (relativeFile) => {
      ...
      for (const rawUrl of source.match(URL_PATTERN) ?? []) {
        const host = (() => {
          try {
        ...
        const hostIsAllowed = allowedHosts.has(host);
