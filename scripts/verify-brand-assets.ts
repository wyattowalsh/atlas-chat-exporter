...
const canonicalSizes = [2048, 1024, 512, 256, 128, 64, 32, 16];
...
function report(message, isWarn = false) {
  if (isWarn) {
    warned = true;
...
for (const rel of requiredBrandFiles) {
  ...
  const expectedSize = Number(rel.match(/logo-(\d+)\.png$/)?.[1]);
...
if (!fs.existsSync(userscriptPath)) {
  ...
  if (!src.match(/@icon\s+.+/)) report('Userscript metadata missing @icon.');
  if (!src.match(/@icon64\s+.+/)) report('Userscript metadata missing @icon64.');
}

if (failed) process.exit(1);
console.log(warned ? '⚠️ Brand asset verification passed with warnings.' : '✅ Brand asset verification passed.');
