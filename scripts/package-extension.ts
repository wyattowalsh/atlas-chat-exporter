...
function parseArgs(argv: string[]): Args {
  ...
  return {
    version,
    extensionDir: params.get("extension-dir") ?? "apps/extension/dist",
    outDir: params.get("out-dir") ?? "dist/release",
    artifactPrefix: params.get("artifact-prefix") ?? "atlas-chat-exporter-extension",
    sourceDateEpoch: Number(params.get("source-date-epoch") ?? process.env.SOURCE_DATE_EPOCH ?? DEFAULT_EPOCH),
...
async function deriveExtensionId(keyPath: string): Promise<string> {
  ...
  const id = Array.from(digest.slice(0, 16), (byte) => `${nibbleToLetter(byte >> 4)}${nibbleToLetter(byte & 0x0f)}`).join("");
  return id;
}

async function makeCrx(args: Args, stagedExtensionDir: string, outDir: string, artifactBase: string): Promise<{ crxPath: string; xmlPath?: string } | null> {
  if (!args.keyPath) {
    console.log("No CRX key provided. Skipping CRX generation; use Chrome Web Store upload as fallback.");
    return null;
  }
...
async function main() {
  ...
  await writeFile(path.join(outDir, `${artifactBase}.zip.sha256`), `${sha}  ${path.basename(zipPath)}\n`, "utf8");
