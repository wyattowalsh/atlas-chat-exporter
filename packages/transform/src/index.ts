import type { Block, CitationMode, ConversationDoc, ExportOptions, ListItem, Turn } from "../../shared/src";
...
function transformTurn(turn: Turn, options: ExportOptions): Turn {
  const blocks = turn.blocks
    .map((block) => transformBlock(block, options))
...
function transformBlock(block: Block, options: ExportOptions): Block | null {
  ...
  if (block.kind === "list") {
    const items = block.items
      .map((item) => transformListItem(item, options))
...
function applyCitationMode(value: string, mode: CitationMode): string {
  ...
  if (mode === "normalize") {
    return value.replace(CITATION_TOKEN, (_, index, _label, href) => {
      const safeHref = decodePipes(href);
      return safeHref ? `[citation ${index}](${safeHref})` : `[citation ${index}]`;
    });
  }

  return value.replace(CITATION_TOKEN, (_, _index, label, href) => {
