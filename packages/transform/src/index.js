import { normalizeBlockWhitespace, normalizeInlineWhitespace } from "@atlas/shared";
...
function applyCitationMode(text, mode) {
    if (mode === "keep") {
        return text;
    }
    if (mode === "strip") {
    ...
    return text.replace(/\[\[citation:(.*?)\]\]/gi, (_full, label) => {
...
export function applyDeterministicTransforms(document, options) {
    const normalizedTurns = document.turns
        .map((turn) => ({
        ...turn,
        blocks: turn.blocks.map((block) => cleanBlock(block, options)).filter((block) => Boolean(block))
    }))
