...
const UI_NOISE_PATTERNS = [/^copy code$/i, /^edit message$/i, /^chatgpt said:?$/i, /^you said:?$/i, /^regenerate$/i];
...
function collectBlockNodes(turnNode: Element): Element[] {
  ...
  const candidates =
    preferred.length > 0 ? preferred : Array.from(turnNode.querySelectorAll("h1,h2,h3,h4,h5,h6,p,pre,blockquote,ul,ol,table,hr"));
