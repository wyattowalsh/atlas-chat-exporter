...
export function exportConversation(html, options = {}) {
  const resolved = { ...defaultExportOptions, ...options };
  const parsed = parseConversationHtml(html);
  if (!parsed.turns.length) throw new NoTurnsFoundError("No conversation turns were found.");
  ...
  const output = resolved.outputFormat === "json" ? renderJson(doc) : renderMarkdown(doc, resolved.includeRoleHeadings, resolved.includeHorizontalRules);
  return { doc, output };
}
