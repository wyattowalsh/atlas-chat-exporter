...
function extractConversationFromPage(documentLike, options = DEFAULT_OPTIONS) {
...
function renderMarkdown(doc, options = DEFAULT_OPTIONS) {
  return doc.turns
    .map((turn, index) => {
...
function exportConversation(documentLike, rawOptions = {}) {
...
const result=exportConversation(document,{outputFormat:'markdown'});const b=new Blob([result.content],{type:result.mimeType});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='chat-export.'+result.extension;a.click();})();